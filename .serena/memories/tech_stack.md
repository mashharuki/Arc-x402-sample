...
- Chain: Arc Testnet (`arcTestnet` from `viem/chains`, `eip155:5042002`) — chain/asset wiring and token EIP-712 gotchas in `mem:chain_config`.
- Payment scheme: `exact` (client/server/facilitator) and `upto` (facilitator only, via `UptoEvmScheme`).
- Payment asset: USDC, address from `ASSET_ADDRESS` env (client + server). Amount and EIP-712 `extra` are hardcoded in `pkgs/server/src/config.ts`.
