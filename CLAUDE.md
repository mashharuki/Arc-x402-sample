...
### Chain/asset config lives in `pkgs/config`

`pkgs/config/src/index.ts` (workspace package `@x402-sample/config`, TS source consumed directly by tsx and wrangler) is the single place for the chain (`CHAIN`), token (`TOKEN`: address, EIP-712 `name`/`version`, `decimals`), prices (`PRICING`, written in token units and converted with `toAtomic`) and limits (`LIMITS`). server, facilitator, client and mcp import it; `ASSET_ADDRESS` and `CHAIN_ID` are no longer env vars or wrangler `vars`. This is the one deliberate exception to "no cross-imports" between packages. `TOKEN.name`/`version` must match the token's on-chain `name()`/`version()`: a wrong `version` makes the facilitator's verify revert with `FiatTokenV2: invalid signature`. Arc USDC is `name="USDC"`, `version="2"`, `decimals=6`; read these from the contract when changing token. Workers must be redeployed after a change. Some human-readable strings (MCP tool descriptions in `pkgs/mcp/src/lib/tools.ts`, comments in `guardrails.ts`) still mention "0.5 USDC" and are not generated from the config.
