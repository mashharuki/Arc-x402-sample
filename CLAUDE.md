...
- Secrets (`EVM_PRIVATE_KEY`, `PRIVY_APP_SECRET`) and per-package values (`FACILITATOR_URL`, `PAYWALL_API_BASE_URL`, `EVM_ADDRESS`, `ALLOWED_PAYEES`, `PRIVY_*`, `PORT`) deliberately stay in each package's own `.env`; `PRIVY_APP_SECRET` must never sit in a file shared with attendees. `EVM_ADDRESS` (server) and `ALLOWED_PAYEES` (mcp) are the same address entered twice and could be unified later.
